-- Add missing subgenres (avoiding duplicates)
...
FROM (VALUES
  ...
  -- Hip-Hop subgenres (skip Trap as it exists)
...
UPDATE public.members SET
  ...
  END,
  families = CASE 
    WHEN name LIKE '%John%' OR name LIKE '%Producer%' THEN ARRAY['Electronic', 'Pop']
    WHEN name LIKE '%Jane%' OR name LIKE '%Artist%' THEN ARRAY['Hip-Hop', 'R&B']
    ELSE ARRAY['Rock']
  END,
  subgenres = CASE 
    WHEN name LIKE '%John%' OR name LIKE '%Producer%' THEN ARRAY['Future Bass', 'Indie Pop']
    WHEN name LIKE '%Jane%' OR name LIKE '%Artist%' THEN ARRAY['Trap', 'Contemporary R&B']
