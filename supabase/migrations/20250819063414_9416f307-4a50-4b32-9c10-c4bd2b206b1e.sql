...
-- Insert some default genre families and subgenres for testing
...
INSERT INTO public.subgenres (family_id, name, patterns, order_index) 
SELECT 
    gf.id,
    subgenre_data.name,
    subgenre_data.patterns,
    subgenre_data.order_index
FROM public.genre_families gf
CROSS JOIN (
    SELECT * FROM (VALUES
        ('Electronic', 'House', ARRAY['house', 'deep house', 'tech house'], 1),
        ('Electronic', 'Techno', ARRAY['techno', 'minimal techno', 'detroit techno'], 2),
        ('Electronic', 'Drum & Bass', ARRAY['drum and bass', 'dnb', 'liquid dnb'], 3),
        ('Hip-Hop', 'Trap', ARRAY['trap', 'hip hop trap', 'southern hip hop'], 1),
        ('Hip-Hop', 'Boom Bap', ARRAY['boom bap', 'east coast hip hop', 'underground hip hop'], 2),
        ('Hip-Hop', 'Lo-Fi Hip Hop', ARRAY['lo-fi', 'chillhop', 'lo-fi hip hop'], 3),
        ('Rock', 'Indie Rock', ARRAY['indie rock', 'alternative rock', 'indie'], 1),
        ('Rock', 'Metal', ARRAY['metal', 'heavy metal', 'death metal'], 2),
        ('Pop', 'Indie Pop', ARRAY['indie pop', 'dream pop', 'bedroom pop'], 1),
        ('R&B', 'Neo-Soul', ARRAY['neo soul', 'neo-soul', 'contemporary r&b'], 1)
    ) AS t(family_name, name, patterns, order_index)
) subgenre_data ON gf.name = subgenre_data.family_name;