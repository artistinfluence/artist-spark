...
-- Drop the computed column first, then the function, then recreate both
ALTER TABLE public.members 
DROP COLUMN computed_monthly_repost_limit;

DROP FUNCTION IF EXISTS public.calculate_repost_limit(integer);
...
-- Recreate the computed column to use manual override
ALTER TABLE public.members 
