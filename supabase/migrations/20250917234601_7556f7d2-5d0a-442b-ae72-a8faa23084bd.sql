-- Analytics and Business Intelligence Database Enhancements - Corrected Enum Values
...
-- Revenue analytics materialized view (using correct enum values)
...
SELECT 
    ...
    COUNT(CASE WHEN c.status = 'live' THEN 1 END) as live_campaigns,
    COUNT(CASE WHEN c.status = 'scheduled' THEN 1 END) as scheduled_campaigns,
