-- First, let's clean up duplicate Hip-Hop entries and organize genres
-- Move all subgenres except rock under Electronic family

-- Get the Electronic and Rock family IDs
DO $$
DECLARE
    electronic_id UUID;
    rock_id UUID;
BEGIN
    -- Get Electronic family ID (create if doesn't exist)
    SELECT id INTO electronic_id FROM public.genre_families WHERE name = 'Electronic' LIMIT 1;
    IF electronic_id IS NULL THEN
        INSERT INTO public.genre_families (name, active) VALUES ('Electronic', true) RETURNING id INTO electronic_id;
    END IF;

    -- Get Rock family ID (create if doesn't exist)  
    SELECT id INTO rock_id FROM public.genre_families WHERE name = 'Rock' LIMIT 1;
    IF rock_id IS NULL THEN
        INSERT INTO public.genre_families (name, active) VALUES ('Rock', true) RETURNING id INTO rock_id;
    END IF;

    -- Move all non-rock subgenres to Electronic family
    UPDATE public.subgenres 
    SET family_id = electronic_id 
    WHERE family_id != rock_id;

    -- Remove duplicate and unused genre families, keeping only Electronic and Rock
    DELETE FROM public.genre_families 
    WHERE name NOT IN ('Electronic', 'Rock');

    -- Add comprehensive electronic subgenres if they don't exist
