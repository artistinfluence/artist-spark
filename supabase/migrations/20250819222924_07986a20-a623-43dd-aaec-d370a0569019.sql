...
-- Create trigger function for inquiry status changes  
...
DROP TRIGGER IF EXISTS inquiry_status_change_trigger ON inquiries;  
CREATE TRIGGER inquiry_status_change_trigger
  AFTER UPDATE ON inquiries
  FOR EACH ROW
  EXECUTE FUNCTION handle_inquiry_status_change();